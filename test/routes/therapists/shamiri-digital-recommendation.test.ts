...
test("POST /therapist/assignment should recommend a cbt Event", async (t) => {
    ...
    const sampleCBTEvent = await generateShamiriDigitalEvent(app.db, sampleUser.id, sampleTopicId);
    ...
    const recommendedCBTEvent = await app.db.query.cbtEvent.findFirst({
        where: eq(cbtEvent.cbtCourseId, parseInt(payload.shamiriDigitalRecommendation))
    })
