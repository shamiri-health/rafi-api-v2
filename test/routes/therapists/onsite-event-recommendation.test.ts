...
import { onsiteEvent, therapySession, user } from "../../../src/database/schema";
...
test("POST /therapists/assignment should recommend a onsite event", async t => {
    ...
    const sampleOnsiteEvent = await generateOnsiteEvent(app.db,sampleUser.id, sampleTherapistId);
