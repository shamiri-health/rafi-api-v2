...
import { dailyCheckIn, rewardHubRecord, user, userAchievement } from "../../../src/database/schema";
import { generateRewardHubRecord, generateUserAchievement } from "../../fixtures/rewardHub";
...
test("POST /daily-check-in should create a new daily checkin record", async (t) => {
    ...
    const sampleAchievement = await generateUserAchievement(app.db, sampleUser.id);
    // Generate a sample reward hub record
    await generateRewardHubRecord(app.db, sampleUser.id);

    t.teardown(async () => {
        await app.db.delete(rewardHubRecord).where(eq(rewardHubRecord.userId, sampleUser.id));
        await app.db.delete(userAchievement).where(eq(userAchievement.id, sampleAchievement.id));
        await app.db.delete(dailyCheckIn).where(eq(dailyCheckIn.userId, sampleUser.id));
    ...
    const userAchievementRecord = await app.db.query.dailyCheckIn.findFirst({
        where: and( 
