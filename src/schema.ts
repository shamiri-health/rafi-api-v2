...
export const alacarteOrder = pgTable("alacarteOrder", {
	id: integer("id").primaryKey().notNull().references(() => order.id),
	userId: integer("userId").references(() => user.id),
...
export const blacklistToken = pgTable("blacklistToken", {
	id: serial("id").primaryKey().notNull(),
...
export const cbtTopic = pgTable("cbtTopic", {
	id: varchar("id", { length: 10 }).primaryKey().notNull(),
	cbtModuleId: varchar("cbtModuleId", { length: 10 }).references(() => cbtModule.id),
	name: varchar("name", { length: 100 }).notNull(),
...
export const goals = pgTable("goals", {
	...
	durationStart: timestamp("duration_start", { withTimezone: true, mode: 'string' }),
	durationEnd: timestamp("duration_end", { withTimezone: true, mode: 'string' }),
...
export const groupSession = pgTable("groupSession", {
	...
	startTime: timestamp("startTime", { mode: 'string' }),
	endTime: timestamp("endTime", { mode: 'string' }),
	therapistId: integer("therapistId").default(8).notNull().references(() => therapist.id),
	groupTopicId: integer("groupTopicId").references(() => groupTopic.id),
...
export const human = pgTable("human", {
	id: serial("id").primaryKey().notNull(),
...
export const quickReplies = pgTable("quickReplies", {
	id: integer("id").primaryKey().notNull().references(() => userResponse.id),
...
export const rewardHubRecord = pgTable("rewardHubRecord", {
	id: serial("id").primaryKey().notNull(),
	userRewardHubId: integer("userRewardHubId").references(() => userRewardHub.id),
	level: integer("level"),
...
export const teletherapyOrder = pgTable("teletherapyOrder", {
	id: serial("id").primaryKey().notNull().references(() => order.id),
	userId: integer("userId").references(() => user.id),
...
export const therapistCal = pgTable("therapistCal", {
...
export const userBaselineRecord = pgTable("userBaselineRecord", {
	id: integer("id").primaryKey().notNull().references(() => userRecord.id),
...
export const userDisplay = pgTable("userDisplay", {
	id: serial("id").primaryKey().notNull(),
	userId: integer("userId").references(() => user.id),
...
export const userService = pgTable("userService", {
	...
	assignedTherapistId: integer("assignedTherapistId").default(205).references(() => therapist.id),
});

export const userSystemResponse = pgTable("userSystemResponse", {
	id: serial("id").primaryKey().notNull(),
	systemResponseId: varchar("systemResponse_id", { length: 20 }).references(() => systemResponse.id),
...
export const friendRequest = pgTable("friendRequest", {
	initiatorId: integer("initiatorId").notNull().references(() => user.id),
	targetId: integer("targetId").notNull().references(() => user.id),
	createdAt: timestamp("createdAt", { mode: 'string' }),
	updatedAt: timestamp("updatedAt", { mode: 'string' }),
},
(table) => {
...
export const friendship = pgTable("friendship", {
	...
	rightId: integer("rightId").notNull().references(() => user.id),
	createdAt: timestamp("createdAt", { mode: 'string' }),
	updatedAt: timestamp("updatedAt", { mode: 'string' }),
},
(table) => {
